    ...
    <body class="antialiased bg-white text-black">
        <div class="relative flex items-top justify-center min-h-screen bg-white text-black sm:items-center sm:pt-0" role="main">
            <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
                ...
                {{--
                    Halaman error ini bisa muncul di tengah alur terkunci (mis. SaaSLifecycleLock)
                    tanpa navigasi aplikasi sama sekali. Tanpa tombol ini, user yang terkunci tidak
                    punya jalan keluar dari UI — harus tahu URL logout dan ketik manual.
                --}}
                @auth
                    <form method="POST" action="{{ auth()->user()->isWaliSantri() ? route('logout') : route('filament.admin.auth.logout') }}" class="mt-6 ml-4">
                        @csrf
