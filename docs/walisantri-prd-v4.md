...
**Last Updated:** Agustus 2026 — v4.60
...
⚠️ **Keduanya sengaja TIDAK lewat rute `admin.export.*`, dan itu keputusan arsitektur bukan kemalasan.** Rute adalah request HTTP baru: ia tidak tahu filter, pencarian, dan sortir yang sedang aktif di tabel Livewire, sehingga berkasnya akan selalu berisi seluruh tabel dan diam-diam berbeda dari yang dilihat di layar — persis pertanyaan "kok isinya beda dengan yang saya lihat" yang mahal dijawab belakangan. Membawa state itu lewat query string berarti memetakan ulang tiap filter satu per satu dan menjaga dua definisi filter tetap sinkron selamanya. Grup rute `admin-export` juga bermiddleware `tenant.resolve`, sementara super admin ber-`pesantren_id = null`. Karena itu responsnya dikembalikan **langsung dari closure `Action::make()->action()`**: `callMountedAction()` meneruskan nilai balik closure, dan Livewire (`SupportFileDownloads`) menangkap `BinaryFileResponse` lalu mengubahnya jadi unduhan. Querynya diambil dari `getTableQueryForExport()` — method yang dipakai `ExportAction` bawaan Filament sendiri, sudah menggabungkan `modifyQueryUsing` + filter + pencarian + sortir dan sudah di-clone.

⚠️ **`->where(...)->oldestOfMany()` adalah jebakan yang tidak menimbulkan galat apa pun, dan relasi `adminUtama()` sempat memakainya.** `CanBeOneOfMany::newOneOfManySubQuery()` membangun query BARU dari model, jadi `where('role','admin_pesantren')` yang dirantai sebelumnya **tidak ikut** ke subquery agregat: yang dihitung `MIN(id)` atas SELURUH user pesantren, lalu outer query membuang baris itu kalau ternyata bukan admin. Relasi mengembalikan NULL padahal adminnya ada — cukup satu ustadz yang lahir sebelum admin (impor massal, atau admin dihapus lalu dibuat ulang) untuk memicunya. Bentuk yang benar `ofMany(['id' => 'MIN'], fn ($q) => $q->where('role', ...), 'admin_utama')->where('role', ...)`: closure diterapkan ke subquery, `where()` luar menyaring outer query, dan alias join dieksplisitkan supaya tidak ditebak lewat `debug_backtrace()`. **Tesnya sengaja membuat ustadz lebih dulu** (`test_kolom_admin_diambil_dari_admin_terlama_meski_ustadz_dibuat_duluan`) — menulis admin duluan, urutan yang paling wajar di tes, menyembunyikan bug ini total.

**`PenerimaEmail::adminPesantren()` sengaja TIDAK dialihkan ke `adminUtama()`.** Dua pertanyaan berbeda: yang satu "siapa admin terlama", yang satu "siapa yang **bisa** diemail" — ia menyaring admin ber-email kosong (`users.email` nullable sejak `central/2026_07_09_100001`) karena `Mail::to(null)` melempar exception. Menukarnya berarti membuat `Mail::to(null)` mungkin lagi.

**Permintaan "kolom aktivitas terakhir" ditolak dan diganti, bukan dipenuhi setengah hati.** Data yang ada tidak bisa menjawabnya dengan jujur: `activity_logs` hanya mencatat enam event siklus langganan (`order.bukti_uploaded/confirmed/rejected`, `pesantren.created/deleted/paket_changed`) sehingga hasilnya praktis selalu sama dengan tanggal daftar — satu kolom duplikat yang terbaca seperti informasi baru; indeksnya pun `(pesantren_id, event)`, bukan `(pesantren_id, created_at)`. `sessions.last_activity` di-garbage-collect, jadi kosong ≠ tidak aktif. Penggantinya dua kolom yang benar-benar ditulis oleh pemakaian sehari-hari: **Santri Terakhir Ditambahkan** (`withMax('santri','created_at')`) dan **Onboarding Selesai**.

**Tiga jebakan lain yang dicatat karena diam saat dilanggar.** (1) **`santri_count_cache` tidak dipakai** — grep seluruh `app/` menunjukkan kolom itu hanya pernah *ditulis* di `OnboardPesantren` (diset `0`) dan `SegarkanSandboxCommand`; observer pemeliharanya yang disebut komentar `PesantrenObserver` tidak pernah dibuat, jadi nilainya nyaris selalu 0 dan berbohong. Jumlah santri dihitung live lewat `withCount`. Dicatat sebagai utang di §22. (2) **`withMax()` mengembalikan string mentah, bukan Carbon** (kolomnya tidak ada di `$casts`) — diformat tanpa di-parse sebagai UTC lebih dulu, jamnya meleset 7 jam tanpa galat. (3) **`Builder::chunk()` milik maatwebsite berbasis offset/limit**, sementara `defaultSort('created_at','desc')` kedua tabel tidak unik; baris ber-`created_at` kembar bisa terduplikasi atau terlewat di batas chunk. Query dari `getTableQueryForExport()` sudah dibubuhi tiebreaker `id` oleh Filament sendiri (`CanSortRecords::applySortingToTableQuery`), tapi kedua kelas Export tetap memasang `orderBy('id')` supaya aman dipakai langsung tanpa query tabel.

**Ekspor Data Pesantren menghormati filter `is_demo` bawaan**, artinya tenant sandbox **tidak** ikut kecuali difilter eksplisit — berkas standarnya terbaca sebagai daftar pelanggan. Itu perilaku yang benar tapi bisa mengejutkan, jadi tombolnya bertooltip dan perilakunya dikunci tes tersendiri.

**Cakupan tes naik jadi 894 tes / 3.915 asersi** (dari 876), satu berkas baru: `SuperAdminExportTest` (18 kasus). Satu di antaranya sengaja **tanpa `Excel::fake()`** dan memakai `assertFileDownloaded()` — ia satu-satunya yang benar-benar melewati rantai Filament action → Livewire → `BinaryFileResponse` → PhpSpreadsheet; sisanya memakai fake dan karena itu tidak membuktikan berkasnya sungguh terbentuk. ⚠️ Keduanya tidak boleh digabung dalam satu tes: `ExcelFake::download()` mengembalikan response tanpa header `Content-Disposition`, sehingga nama berkasnya jatuh jadi `'download'`.

---

**Changelog v4.57:** **Pesantren akhirnya bisa mematikan modul yang memang tidak ia pakai — enam tuas, satu per cluster sidebar.** `Akademik · Tahfidz · Presensi · Kesantrian · Keuangan · Rapor`, disimpan di tabel tenant baru `modul_pengaturan` dan disunting admin pesantren lewat halaman **Manajemen → Modul** (`/admin/pengaturan-modul`). Cluster **Santri** (Santri/Kelas/Kamar/Prestasi) sengaja tidak punya tuas: ia inti sistem.

⚠️ **Ini BUKAN feature lock berbasis paket, dan bedanya bukan sekadar penamaan.** Yang memutuskan adalah admin pesantren, bukan platform; seluruh default `true` sehingga tidak ada satu pun tenant lama yang berubah setelah migrasi; dan tidak ada logika paket yang membaca atau menulis kolomnya. "Feature lock berbasis paket" tetap tinggal di daftar di-skip §22 — lihat paragraf baru di §5.1.

**Pemicunya justru sebuah penolakan.** Permintaan awalnya menyembunyikan menu presensi dari ustadz yang bukan wali kelas. Itu ditolak: visibilitas menu tidak boleh mengikuti data FK yang berubah sendiri — `kelas.wali_kelas_id` berpindah di tengah semester, menunya lenyap tanpa penjelasan, dan empty-state `PresensiHarianPage` ("minta admin menetapkan Anda lewat Santri → Kelas") yang selama ini satu-satunya jalur self-service ustadz ikut hilang bersamanya. Keamanannya pun sudah dijaga `canAccess()` + `ScopesQueryToPresensiUstadz`, jadi menyembunyikan menu tidak menambah proteksi apa pun. Yang keluhannya sah adalah panel yang memuat modul yang memang tidak dipakai pesantren — dan tuas yang tepat untuk itu berlaku **per pesantren**, bukan per orang, sehingga menu tetap seragam antar-pengguna dan tiket "menu saya kok beda dengan teman saya" tidak pernah lahir.

**Satu metode menggerakkan seluruh permukaan panel, dan itu yang membuat diff-nya kecil.** `Cluster::canAccessClusteredComponents()` menanyakan `canAccess()` tiap komponen; sidebar memakainya lewat `shouldRegisterNavigation()`, dan `bottom-nav.blade.php` memanggilnya langsung. Untuk Resource, `registerNavigationItems()` juga berhenti bila `canAccess()` false. Lebih jauh, `CanAuthorizeAccess` milik Filament menjaga **`hydrate`, bukan hanya `mount`** — jadi tab yang terlanjur terbuka dan request Livewire yang dirakit tangan ikut tertutup 403 tanpa satu baris middleware pun di panel admin. Dari 27 komponen, 15 mewarisi gate-nya dari dua trait bersama (`HasAdminUstadzAccess`, `HasAdminOnlyAccess`), jadi yang disunting cuma dua trait + 14 baris.

⚠️ **Preseden `PresensiJamPage` sengaja diputus, dan alasannya mekanis bukan selera.** Toggle `presensi_per_jam_aktif` sengaja TIDAK dinilai di `canAccess()` supaya admin tidak menabrak 403 telanjang. Toggle modul tidak bisa mengikutinya: `canAccess()` adalah metode yang **sama** yang memutuskan "menu tampil" dan "URL terjangkau" — tidak mungkin membiarkan URL terbuka sambil membuat menunya hilang. Preseden lama tetap sah justru karena toggle per-jam hidup **di dalam** modul yang menunya tetap ada. Peredamnya: halaman Pengaturan Modul tidak pernah ikut dimatikan (aman by construction — ia tanpa cluster) sehingga penjelasannya selalu satu klik, dan tiap `helperText` menyebut persis menu apa yang lenyap.

**Portal wali ikut, dengan 404 dan bukan 403.** Penjaganya middleware rute `modul:{nama}` yang dipasang deklaratif di `routes/web.php` — sejajar `signed`/`throttle:*`/`magic.token` yang sudah tertulis di sana, dan ia menutup POST (`spp.konfirmasi`, `izin.store`) yang tidak bisa dijaga lapis view. 404 dipilih karena bagi orang tua santri, modul yang pesantrennya tidak pakai memang **tidak ada**; 403 justru mengumumkan keberadaan sesuatu yang sedang ditolak. Query-nya juga benar-benar tidak dijalankan: `SantriDetailPresenter::detail()` melewati grup query milik modul yang mati alih-alih menyembunyikannya di Blade, sambil menjaga seluruh kunci `compact()` tetap ada. ⚠️ Pemeriksaan modul di sana membaca `$santri->pesantren_id`, **bukan** konteks tenant — rute `wali.magic.report` tidak memakai `tenant.resolve`, dan salahnya tidak memunculkan galat apa pun, wali hanya melihat seksi milik pesantren lain.

**Tiga jebakan yang dicatat karena tidak akan menimbulkan galat bila terlanggar.** (1) Properti Livewire toggle wajib `?bool`, bukan `bool` — Toggle yang kembali null memecahkan render sebelum validasi sempat bicara (kelas bug v4.28/v4.39). (2) Memoisasi `ModulPengaturan::untuk()` hidup di **container**, bukan `private static array`: static array bertahan melewati `RefreshDatabase` antar-method di proses yang sama, sehingga tes kedua membaca toggle milik tes pertama dan gagal dengan gejala yang terbaca seperti bug produk, di berkas yang bukan berkas yang disunting. (3) `HasAdminOnlyAccess` juga dipakai `KelasResource` & `KamarResource` yang **inti** — yang menjaganya cuma fakta bahwa modul diturunkan dari **cluster** dan Cluster Santri tidak punya modul.

**`RaporPage`: `mutabaah` DAN `karakter` sama-sama menggantung ke tuas Kesantrian** (keduanya resource di cluster itu), jadi satu tuas menghilangkan dua centang. Penjagaan request basi duduk di satu baris di `isModulAktif()` — `$modul` properti Livewire publik, jadi tab lama bisa membawa kunci yang sudah tidak ada di opsi; karena `getData()` menyaring setiap sumber lewat method itu, satu pemeriksaan menutup layar dan PDF sekaligus.

**Penjaga kelengkapan menggantikan middleware.** PHP tidak punya mekanisme level-kelas yang anti-override (method di kelas mengalahkan trait tanpa peringatan), jadi `ModulNavigasiTest::test_setiap_komponen_cluster_ikut_dimatikan` mengenumerasi `Cluster::getClusteredComponents()` dan menuntut `canAccess() === false` untuk **setiap** komponen. Resource yang ditambahkan ke sebuah cluster tahun depan tercakup sejak hari ia ditulis. Sebelas widget dashboard satu-modul ikut digating, dan `AdminStatsOverview`/`UstadzStatsOverview` disaring per-`Stat` — `AdminSppStatusChart` menaut ke `TagihanSppResource`, jadi membiarkannya tampil saat Keuangan mati menghasilkan kartu yang satu-satunya afordansinya halaman 403.

**Cakupan tes naik jadi 853 tes / 3.362 asersi** (dari 830) — lima berkas baru: `ModulPengaturanTest`, `ModulPengaturanPageTest`, `ModulNavigasiTest`, `ModulRaporPageTest`, `ModulWidgetTest`, `WaliModulTest`, plus `tests/TenantIsolation/ModulIsolationTest`. **Celah yang sengaja dibiarkan terbuka:** `ExportController` (`admin-export/presensi`, `admin-export/mutabaah`) tidak digating — hanya terjangkau admin yang mematikan modulnya sendiri, tapi dicatat di sini sebagai gap terdokumentasi, bukan kelalaian.
...
**Angkanya tetap satu sumber.** `PaketHargaService::kartuUntukDurasi()` baru melayani halaman ini; `kartu()` yang dipakai `/harga` sengaja tidak disentuh karena bentuknya berbeda — di sana kedua siklus dirender sekaligus lalu salah satunya disembunyikan CSS (landing & `/harga` nol JavaScript), sedangkan di panel siklusnya state Livewire sehingga cukup satu. Aturan v4.41 berlaku penuh: nol angka rupiah ditulis di Blade, dan `UpgradePagePilihanPaketTest` menggeser `harga_tumbuh`/`kuota_tumbuh` ke nilai tidak lazim untuk membuktikannya.

⚠️ **Dua pagar ikut berpindah bersama komponennya, dan itu bagian paling mudah hilang dari perubahan ini.** (1) Daftar durasi dulu difilter di `->options()` milik `Select` — begitu dropdown-nya diganti tombol, batas `min_durasi_upgrade` (§16) harus hidup di `pilihDurasi()`, bukan lenyap; tombol yang tidak memenuhi syarat kini **ditandai `disabled` beserta alasannya**, bukan dihilangkan, karena tombol yang lenyap tanpa penjelasan terbaca sebagai "opsinya tidak ada" padahal yang benar "sisa langganan Anda masih panjang". (2) `pilihPaket()` menolak paket yang kuotanya di bawah santri aktif — kartunya sudah `disabled` di Blade, tapi `wire:click` tetap bisa dipanggil langsung dari klien. Pagar v4.53 (`kuotaKurang()` → tombol bayar mati + callout merah, dan penolakan di `prosesPembayaran()`) tetap berdiri sebagai lapisan terakhir.

**Palet CSS dinaikkan satu tingkat.** Variabel `--us-*` (light + dark) selama ini menempel pada `.upgrade-summary`. Kartu hidup di luar kolom itu, jadi paletnya pindah ke pembungkus `.upgrade-page`; tanpa langkah itu seluruh kartu kehilangan warnanya di dark mode — kelas kegagalan yang tidak akan terlihat di tes mana pun.

**Yang tersisa di schema Filament: kuota Maju dan kode kupon.** Section "Kuota Santri" hanya muncul saat paket Maju terpilih (kuotanya add-on per 100 santri, §5.3), dan `pilihPaket('maju')` menaikkan kuota target ke minimal 1.000 — perilaku yang dulu tertanam di `afterStateUpdated` Select. Durasi 3 bulan **tetap ditawarkan**: menghapusnya berarti mencabut opsi pembelian, bukan merapikan tampilan.

**Changelog v4.53:** **Trial tidak lagi selalu Rintisan — pendaftar memilih paketnya di depan, dan trial-nya lahir pada paket itu.** Sampai v4.52 setiap pendaftar mandiri mendarat di `paket_langganan='rintisan'` dengan kuota 100, berapa pun besar pesantrennya; pesantren 300 santri karena itu menabrak `SantriQuotaExceededException` sebelum sempat melihat nilai produknya, dan trial-nya tidak pernah mewakili paket yang akan dibeli. Kini kartu `/harga` membawa paketnya ke form (`/register?paket=tumbuh`), `/register` menampilkannya sebagai badge di atas form, dan `OnboardPesantren` menuliskan paket itu apa adanya. Durasinya tidak berubah (`BillingSetting::trial_days`), begitu pula lifecycle trial → grace 7 hari → suspended.

⚠️ **Ini perubahan mekanik, BUKAN pembalikan keputusan pemasaran v4.45.** Kata "trial" tetap tidak muncul di `/harga` maupun `/register` — CTA kartu tetap berbunyi "Daftar Sekarang", badge di form hanya menyebut nama paket dan kuotanya. Ketiga tes `assertDontSee('trial')` (`LandingPageTest`, `HargaPageTest`, `RegisterControllerTest`) **tetap berdiri tanpa disunting**, dan yang terakhir diperluas ke varian `?paket=tumbuh` karena badge itu permukaan baru tempat janji lama paling mudah menyelinap kembali. Asimetri v4.45 dipertahankan: pendaftar menemukan trial-nya setelah masuk, bukan dijanjikan sebelum masuk.

**Paket Maju dikecualikan, dan pengecualiannya kini punya satu rumah.** Kuota Maju dinegosiasikan lewat add-on per 100 santri, jadi ia tidak bisa lahir dari form yang tidak punya cara menanyakannya. `PaketLangganan::bisaDipilihSendiri()`/`pilihanMandiri()` melayani keempat pemakainya sekaligus — kartu `/harga` (menggantikan perbandingan `=== PaketLangganan::Maju` di `PaketHargaService`), rule validasi `/register`, daftar radio di form, dan aksi ganti paket trial. Penolakannya ditegakkan **di server** (`Rule::in`) plus pagar terakhir `InvalidArgumentException` di `OnboardPesantren`, bukan sekadar tidak dirender: `/register?paket=maju` diabaikan diam-diam dan form kembali meminta pendaftar memilih.

**`/register` jadi tiga langkah: Paket · Data Pesantren · Penanggung Jawab.** Langkah 1 dilewati otomatis bila paket sudah dibawa dari `/harga` — `$langkahAwal` di Blade kini bercabang tiga (galat `paket` → 1, galat kolom pesantren → 2, sisanya → 3), pola yang sama dengan v4.51. `/register` **tetap berdiri sendiri**: tombol "Daftar" di hero landing, CTA penutup, nav, dan footer tidak diubah dan tidak pernah mendarat di halaman buntu, jadi tautan serta bookmark lama tetap hidup. Wizard JS-nya digeneralkan (`langkahAktif` + `TERAKHIR`) alih-alih menghardcode dua panel.

**Kuota trial dibaca lewat `BillingCalculatorService`, bukan `PaketLangganan::maxSantri()`.** Angka di enum itu hardcode 100/250/500/1000 dan akan menyimpang begitu super admin menggeser kuota di Pengaturan Harga — sumber yang sama yang sudah dipakai `/harga`. Dijaga `test_trial_lahir_pada_paket_pilihan_dengan_kuota_dari_billing_setting`, yang menggeser `kuota_tumbuh` ke 275 lebih dulu: kalau kuota kembali diambil dari enum, tes itu merah.

⚠️ **Arah paket kini bisa TURUN, dan itu melahirkan keadaan yang sebelumnya mustahil.** Selama trial selalu Rintisan (kuota terkecil), paket hanya bisa naik. Sekarang tenant bisa trial Berkembang, mengisi 300 santri, lalu membeli Rintisan. `SantriObserver` hanya menahan **penambahan** santri; tidak ada apa pun yang membereskan kelebihan yang terlanjur ada. Karena itu §16 yang selama ini menulis "santri aktif > kuota baru → downgrade diblokir" akhirnya **benar-benar ditegakkan**: `UpgradePage::kuotaKurang()` mematikan tombol bayar, menampilkan callout merah, dan `prosesPembayaran()` menolak lewat Notification — pagar terakhir yang perlu karena aksi Livewire tetap bisa dipanggil walau tombolnya mati. Kedua lapisan diuji terpisah di `PaketTrialKuotaTest`, sebab asersi tombol akan menghentikan rantai tes sebelum pagar servernya sempat dipanggil.

**Ganti paket selama trial: kuota ikut, sisa hari tidak direset.** Aksi baru "Ganti Paket Trial" di halaman Langganan (`BillingPage`), hanya tampil saat `status_berlangganan='trial'`. Perkiraan pendaftar bisa meleset begitu data santri sungguhan masuk, dan menyuruhnya mendaftar ulang berarti membuang tenant yang sudah diisi. `expired_at` **sengaja tidak disentuh** — kalau ia ikut direset, mengganti paket berulang kali menjadi trial tanpa batas. Aturan kuota yang sama berlaku, dan perubahan paketnya sudah dicatat `PesantrenObserver` (`pesantren.paket_changed`) tanpa kode audit tambahan.

**Catatan operasional:** kolom "Paket" di `TenantListWidget`/`PesantrensTable` kini bisa berisi Berkembang untuk tenant yang belum pernah membayar sepeser pun. Membaca "berapa pelanggan paket X" wajib disaring `status_berlangganan='active'`. Tidak ada perubahan kode untuk itu di rilis ini — dicatat di sini supaya tidak terbaca sebagai angka pendapatan.
...
**Wizard-nya tetap satu `<form>` dan satu POST.** Dua kontainer `[data-langkah]` bergantian disembunyikan oleh JS biasa (halaman ini tidak punya Livewire dan tidak diberi Livewire); payload, aturan validasi, dan penangan `store()` sama persis dengan sebelum wizard ada. Langkah yang terbuka saat validasi gagal **dihitung di Blade** (`$langkahAwal`, dirender sebagai `data-langkah-awal`), bukan oleh JS setelah muat — tanpa itu pendaftar melihat kedipan langkah yang salah, dan tesnya tidak punya apa pun untuk diperiksa. ⚠️ Satu jebakan yang wajib dijaga: kolom `required` di dalam kontainer `display:none` membuat submit **diblokir tanpa pesan apa pun** (browser hanya menulis warning di konsol karena kolomnya tidak bisa difokuskan) — pagar untuk itu adalah listener `invalid` bertahap-capture yang melompat dulu ke langkah pemilik kolom.

**Tabel `wilayah` baru di central (§3.1), tapi isinya sengaja TIDAK disemai lewat migrasi.** 91.599 baris (38 provinsi, 514 kab/kota, 7.285 kecamatan, 83.762 desa/kelurahan) dari `cahyadsn/wilayah` (MIT, Kepmendagri No. 300.2.2-2138/2025), di-vendor sebagai `database/data/wilayah.csv.gz` (±640 KB) supaya VPS dan CI tidak pernah bergantung pada GitHub raw saat deploy. Alasan utamanya bukan biaya tes — `RefreshDatabaseState::$migrated` statis per proses, jadi migrasi hanya jalan sekali untuk seluruh suite — melainkan bahwa **migrasi cuma jalan sekali di production**: setiap rilis dataset Kemendagri berikutnya akan menuntut migrasi kesekian, dan `migrate:refresh` di production mustahil. Pemuatnya `php artisan wilayah:impor`, idempoten (bandingkan `COUNT` dengan metadata berkas → keluar dalam ~50 ms) dan transaksional, dipanggil `deploy.yml` tepat setelah `migrate --force` serta oleh `composer setup`. ⚠️ Langkah deploy itu sengaja **tanpa** `|| true`, berbeda dari langkah backup dan `sandbox:segarkan` di sekitarnya: `/register` mustahil dilalui bila tabelnya kosong, jadi kegagalannya harus menghentikan deploy — `trap 'php artisan up' EXIT` tetap mengembalikan situs online.

⚠️ **437 nama desa memuat apostrof ter-escape gaya MySQL** (`'Pasi Kuala Ba''u'`). Parser `grep`/`sed` naif memotongnya di tengah dan menghasilkan data rusak **tanpa galat apa pun** — itulah kelas kegagalan paling mungkin di seluruh jalur ini, dan alasan regenerasi berkasnya berupa command PHP (`php artisan wilayah:regenerasi`, dev-only, hasilnya di-commit) alih-alih one-liner shell.

**Kolomnya `parent_kode` eksplisit, bukan `LIKE 'kode.%'`.** Dua sebab: pada kolasi non-`C` (default `en_US.UTF-8`) index btree biasa **tidak** dipakai PostgreSQL untuk `LIKE 'prefix%'` — ia menuntut `varchar_pattern_ops`, mudah lupa, dan gagalnya senyap berupa seq scan 91 ribu baris; dan `LIKE '32.01.%'` tetap ikut menjaring cucu. Kesetaraan `parent_kode = ?` bebas dari keduanya dan berperilaku identik di SQLite maupun PostgreSQL. Provinsi ber-`parent_kode` NULL, sehingga satu index `(parent_kode, nama)` melayani "anak dari X" maupun "daftar provinsi" tanpa langkah sort.

**Server tidak pernah percaya induk yang dikirim klien.** Kode desa `32.01.01.1006` sudah memuat seluruh leluhurnya, jadi `WilayahLookup::jalurDariDesa()` menurunkannya sendiri — satu `whereIn` untuk empat level, dimemo per-request sehingga rule validasi dan perakit `profil` berbagi hasil yang sama (total: satu query). `WilayahJalurValid` lalu **mengadu** tiga kode yang dikirim form dengan hasil turunan itu; ketidakcocokan ditolak terang-terangan, bukan diam-diam "diperbaiki" menjadi wilayah yang tidak pernah dipilih pendaftar. Aturan yang sama dipakai `PesantrenSettingsPage`, jadi kombinasi tidak konsisten mustahil tersimpan dari permukaan mana pun.

**Nomor WhatsApp admin akhirnya tersimpan — dan itu menghidupkan fitur yang selama ini mati.** `OnboardPesantren::execute()` sudah punya parameter `$adminPhone` sejak lama, tapi `RegisterController` tidak pernah mengisinya; akibatnya `users.phone_number` selalu kosong untuk pendaftar mandiri, dan `WarnExpiringTenantsWhatsApp`, `CheckExpiredTenants`, serta `UpgradeOrderService` semuanya `return` lebih awal lewat `blank($admin->phone_number)`. Peringatan kedaluwarsa efektif tidak pernah sampai ke satu pun pesantren yang mendaftar sendiri. Kini nomornya wajib, divalidasi `NomorWhatsApp` (regex `no_hp` milik form demo **plus** kepastian `FonnteWhatsAppService::normalizePhoneNumber()` tidak `null`), dan disimpan ternormalisasi `62…` seperti jalur impor Excel.

**`profil['alamat']` kini berisi alamat jalan saja, dan diisi manusia — tidak pernah dirangkai dari wilayah.** Empat kolom wilayah menjawab "di wilayah administratif mana", kolom Alamat menjawab "di jalan mana"; keduanya berdampingan, bukan saling menggantikan. Merangkai `alamat` otomatis dari wilayah sempat dipertimbangkan lalu ditolak: nilainya akan ditimpa begitu admin mengisi alamat sungguhan — dua sumber kebenaran untuk satu key — dan `alamat` adalah bukti "profil sudah diisi manusia" bagi `PesantrenObserver` serta `BackfillOnboardingSteps` (§14), bukti yang jadi bohong kalau diisi mesin. Karena `alamat` sekarang cuma potongan jalan, header profil publik memakai `Pesantren::alamatLengkap()` yang **menggabungkan** keduanya; tenant lama yang tidak punya `wilayah` tetap menampilkan alamat panjangnya apa adanya, tanpa duplikasi.

**Langkah onboarding Profil tetap terbuka setelah pendaftaran**, karena ia menuntut `alamat` **dan** `logo` — dan logo memang belum bisa diunggah dari `/register`. Dampak ke `PesantrenObserver` nol: ia hanya punya `updated()`, sedangkan registrasi memanggil `create()`. Dikunci `RegisterWilayahTest::test_onboarding_profil_belum_selesai_walau_alamat_sudah_terisi`.

⚠️ **Gerbang tombol "Lanjut" harus ikut menyapu `<textarea>`.** Loop `checkValidity()` semula hanya memeriksa `input, select`; kolom Alamat berupa textarea, sehingga tanpa perbaikan itu kolom wajib bisa dilewati begitu saja ke langkah 2 — dan baru ditolak server setelah pendaftar mengisi seluruh langkah 2.

⛔ **Key `email_kontak` yatim sejak lama.** `resources/views/public/profile.blade.php` sudah merender `profil['email_kontak']`, tapi tidak ada satu pun form yang menulisnya — dan seksi Kontak yang membungkusnya hanya muncul bila `telepon` atau `website` terisi. Artinya pesantren yang hanya punya email akan kehilangan seluruh seksi itu. Rilis ini mengisi key tersebut dari kolom "Email Pesantren" di `/register` **dan** memperbaiki kondisi `@if`-nya.

**`PesantrenSettingsPage` ikut mendapat empat Select berjenjang + kolom Email Pesantren — dan tes pertamanya.** Halaman itu sebelumnya sama sekali tidak diuji padahal ia satu-satunya penulis `profil`; `PesantrenSettingsWilayahTest` kini mengunci tiga hal: hidrasi dari `profil` saat mount, nama wilayah diambil ulang dari tabel (bukan dari kiriman klien) saat simpan, dan desa yang dikosongkan **tidak** menghapus wilayah lama — tanpa pagar terakhir itu, satu simpan pada seksi lain akan menghilangkan lokasi diam-diam. — tanpa itu salah pilih saat mendaftar bersifat permanen, karena tidak ada permukaan lain yang bisa mengubah `profil['wilayah']`, dan pendaftar mengisinya di 20 detik pertama hidupnya bersama produk. Opsi terbesar adalah daftar desa satu kecamatan (≤ 100 baris), jadi `->options()` biasa + `searchable` sudah cukup; tabelnya boleh 91 ribu baris tanpa `getSearchResultsUsing()`. ✅ **`PesantrenForm` (create super admin) sengaja TIDAK ikut** — form itu tidak menyentuh `profil` sama sekali; super admin membuat cangkang tenant, admin pesantren melengkapi wilayahnya di Pengaturan. Dicatat di sini supaya tidak muncul sebagai temuan audit berikutnya.

**Angka sebaran tes di §17 dikoreksi, bukan sekadar ditambah.** Blok pohonnya masih menulis `tests/Feature/` 52 berkas dan `tests/Unit/` 3 berkas — keduanya sudah lama basi (nyatanya 82 dan 7 sebelum rilis ini). Pola drift yang sama pernah terjadi pada angka versi di v4.47, v4.49, dan v4.50.

⚠️ **Dua jebakan yang tertangkap saat implementasi, keduanya hanya muncul di satu driver/proses.** (1) Model Eloquent yang di-`Cache::remember` ke store `database` bangkit kembali sebagai `__PHP_Incomplete_Class` di proses pembaca lain — `Wilayah::provinsi()` karena itu meng-cache array polos, bukan model. (2) `profil` bertipe **jsonb**, dan PostgreSQL menormalkan urutan key saat menyimpan sementara SQLite mempertahankannya — `assertSame` atas `profil['wilayah']` hijau di lokal lalu merah di CI. Tes barunya memakai `assertEquals`, dengan alasannya ditulis di tempat.
...
5. **Tuas modul** *(v4.57, menggantikan butir lama)*: bila modul layak dimatikan sendiri oleh pesantren, tambah satu case di `App\Enums\Modul` + kolom `{modul}_aktif` di `modul_pengaturan`, lalu daftarkan komponennya **di dalam sebuah Cluster** — modulnya diturunkan dari cluster (`ModulKomponen`), jadi gate-nya melekat tanpa suntingan tambahan dan `ModulNavigasiTest::test_setiap_komponen_cluster_ikut_dimatikan` langsung ikut menjaganya. ⚠️ Butir ini dulu berbunyi "Gate `access-modul-{x}` di `AppServiceProvider` … bila modul berbayar/terkunci paket" — sudah basi sejak v4.20 (kelima Gate itu dihapus karena tidak pernah dipanggil, §5.1), dan tuas modul **bukan** penggantinya sebagai penguncian paket.
...
**`wilayah`** *(v4.51)* — `kode` PK varchar(13) (`'32'` provinsi / `'32.01'` kab-kota / `'32.01.01'` kecamatan / `'32.01.01.1006'` desa-kelurahan) · `nama` varchar(100) · `parent_kode` varchar(13) null (**NULL = provinsi**) · `level` smallint 1–4 · **tanpa timestamps** (data referensi statis, diganti wholesale). *Index: `(parent_kode, nama)` — satu index melayani "anak dari kode X" maupun "daftar provinsi", karena btree PostgreSQL ikut mengindeks NULL, dan `ORDER BY nama` terlayani tanpa langkah sort.* Referensi wilayah administratif Kemendagri, 91.599 baris dari `cahyadsn/wilayah` (MIT, Kepmendagri No. 300.2.2-2138/2025), di-vendor sebagai `database/data/wilayah.csv.gz`. **Isinya sengaja tidak disemai lewat migrasi** — pemuatnya `php artisan wilayah:impor` (idempoten, transaksional), dipanggil `deploy.yml` setelah `migrate --force` dan oleh `composer setup`; pembaruan dataset lewat `php artisan wilayah:regenerasi` lalu commit ulang berkasnya. Read-only bagi aplikasi. Dipakai `/register` (§4.1) dan `PesantrenSettingsPage`; hasil pilihannya disimpan **didenormalisasi** di `pesantrens.profil['wilayah']` (kode **dan** nama per level), **bukan** sebagai FK — profil publik, ekspor, dan email karena itu tidak pernah butuh join.

**`demo_requests`** — `id` PK · `nama_pesantren` · `nama_kontak` · `email` · `no_hp` · `jumlah_santri` null · `kota` null · `catatan` text null · `contacted_at` ts null (diisi admin saat pesantren dihubungi) · timestamps. *Tabel central, diisi dari halaman `/demo` di landing page; dikelola `DemoRequestResource` hanya `super_admin`. **v4.58:** punya ekspor Excel (`AntreanDemoExport`, §15).*
...
### Pengaturan Modul *(v4.57)*

**`modul_pengaturan`** — `id` PK · `pesantren_id` FK→pesantrens cascadeOnDelete · `akademik_aktif` · `tahfidz_aktif` · `presensi_aktif` · `kesantrian_aktif` · `keuangan_aktif` · `rapor_aktif` — keenamnya boolean **default `true` di DB** · timestamps. *Unique: `pesantren_id` (nama pendek: `modul_pengaturan_ps_unik`).* Migrasi `tenant/2026_08_21_000001` (create) + `000002` (`seed_modul_pengaturan_untuk_pesantren_lama`, raw `DB::table`, idempoten, `down()` sengaja kosong). Satu baris per pesantren, dibaca lewat `ModulPengaturan::untuk($pesantrenId)` yang memoisasi hasilnya **di container** — bukan `Cache::` (toggle basi setelah admin membaliknya adalah kelas kerusakan senyap), dan bukan `private static array` (bocor antar-method tes yang berbagi proses PHP). Tiga lapis pengisi wajib seperti `presensi_pengaturan`: `ProvisionTenant` → migrasi penambal → accessor penyembuh diri.

Satu kolom per **cluster sidebar**, bukan per menu: pesantren memutuskan "kami tidak menjalankan tahfidz", bukan "kami mau Setoran tapi tidak mau Ujian". Cluster **Santri** sengaja tanpa kolom — ia inti sistem dan tuas untuk mematikannya tidak boleh pernah ada. Dibaca lewat `App\Enums\Modul` (yang sengaja tidak tahu apa pun tentang kelas Filament, supaya tetap bisa dipakai dari `routes/web.php` dan Blade wali) dan dipetakan ke komponen panel lewat `App\Filament\Support\ModulKomponen` yang **menurunkan modul dari cluster**. ⚠️ Bukan feature lock paket — lihat §5.1.
...
Via `walisantri.com/register`. **Formnya tiga langkah *(v4.53; dua sejak v4.51)*:** langkah 1 **Paket** (kartu radio Rintisan/Tumbuh/Berkembang, otomatis terlewati bila pendaftar datang dari kartu `/harga` lewat `?paket=`), langkah 2 **Data Pesantren** (nama, subdomain, provinsi → kota/kabupaten → kecamatan → desa/kelurahan sebagai dropdown berjenjang dari tabel `wilayah` §3.1, alamat jalan, plus telepon & email pesantren yang keduanya opsional), langkah 3 **Penanggung Jawab** (nama admin, email, nomor WhatsApp, sandi). Ketiganya hidup dalam satu `<form>` dan satu POST — langkah hanya disembunyikan bergantian oleh JS, jadi validasi server tetap satu permintaan; langkah yang terbuka saat validasi gagal dihitung di Blade (`data-langkah-awal`). Sistem otomatis: (1) validasi slug (format, unik, reserved, cooldown) real-time; (2) buat baris `pesantrens` di central, **termasuk `profil` awal** (`wilayah` empat level berisi kode dan nama, `alamat` jalan, serta `telepon`/`email_kontak` bila diisi); (3) buat baris `tenant_domains` (`type=subdomain`, `{slug}.walisantri.com`); (4) **aktifkan situs profil publik** di subdomain itu (template minimal); (5) buat user pertama role `admin_pesantren`, dengan `phone_number` terisi bentuk ternormalisasi `62…` *(v4.51 — sebelumnya selalu kosong untuk pendaftar mandiri, dan itulah yang membuat ketiga jalur notifikasi WhatsApp kedaluwarsa diam-diam `return` lebih awal)*; (6) aktifkan **trial 14 hari pada paket yang dipilih pendaftar** *(v4.53 — sebelumnya selalu Rintisan)*: `paket_langganan` dari langkah 1, `status_berlangganan='trial'`, `max_santri_kuota` dari `BillingCalculatorService::hitungUntukTarget()` (bukan `PaketLangganan::maxSantri()` yang hardcode), `expired_at=+14 hari` dengan durasi dari `BillingSetting::trial_days`; **paket Maju dikecualikan** — kuotanya dinegosiasikan, jadi `Rule::in(PaketLangganan::pilihanMandiri())` menolaknya dan `OnboardPesantren` melempar `InvalidArgumentException` sebagai pagar terakhir; (7) **kirim email sambutan** ke admin pertama, berisi tautan konfirmasi alamat — di luar transaksi, setelah commit (§12.2); (8) redirect ke `app.walisantri.com/admin`.

> **Kode wilayah tidak pernah dipercaya dari klien *(v4.51)*.** Kode desa sudah memuat seluruh leluhurnya, jadi `WilayahLookup::jalurDariDesa()` menurunkan provinsi/kota/kecamatan sendiri (satu `whereIn`, dimemo per-request) dan `WilayahJalurValid` mengadu tiga kode kiriman form dengan hasil turunan itu. Tidak cocok = ditolak, bukan diperbaiki diam-diam. Aturan yang sama berlaku di `PesantrenSettingsPage`.

> **`profil['alamat']` = alamat jalan saja, dan wajib diketik manusia *(v4.51)*.** Ia melengkapi empat kolom wilayah, bukan menggantikannya, dan tidak pernah dirangkai otomatis dari wilayah — `alamat` adalah penanda "profil sudah diisi manusia" bagi checklist onboarding (§14), jadi mengisinya dengan mesin membuat penandanya bohong. Header profil publik memakai `Pesantren::alamatLengkap()` yang menggabungkan `alamat` + `alamatWilayah()`. Langkah onboarding Profil tetap terbuka setelah pendaftaran karena masih menuntut `logo`.
...
> **Diperbarui v4.53 — yang berubah mekaniknya, bukan tabel di atas.** Trial kini mengikuti paket pilihan pendaftar, tapi seluruh baris tabel ini tetap berlaku: `/harga` dan `/register` tidak menyebut kata "trial", angka hari, maupun "gratis". Badge paket di form hanya memuat nama paket dan kuotanya. Ketiga tes `assertDontSee` tetap berdiri.
...
> **⚠️ Toggle modul per-pesantren (v4.57) BUKAN feature lock paket.** Sejak v4.57 admin pesantren bisa mematikan enam modul untuk pesantrennya sendiri (`Akademik · Tahfidz · Presensi · Kesantrian · Keuangan · Rapor`, tabel `modul_pengaturan`, halaman Manajemen → Modul). Tiga hal yang membedakannya dari matriks di atas, dan ketiganya perlu dibaca sebelum ada yang menyangka penegakan paket sudah dimulai: **(1) yang memutuskan adalah admin pesantren, bukan platform** — ini pesantren memilih keluar, bukan platform mengunci masuk; **(2) seluruh default `true`**, dan migrasi penambalnya mengisi `true` untuk semua tenant lama, jadi tidak ada satu pelanggan pun yang kehilangan akses karena rilis ini; **(3) tidak ada logika paket yang membaca atau menulis kolomnya** — `PaketLangganan` tidak punya method apa pun yang menyentuhnya. **"Feature lock berbasis paket" tetap tinggal di daftar di-skip §22** dan tetap merupakan keputusan bisnis yang belum diambil. Bila kelak keduanya ada, toggle ini di-`AND`-kan **di bawah** pemeriksaan paket — pesantren boleh mematikan yang ia punya, tidak boleh menyalakan yang tidak ia beli.
...
> *Tidak ada paket Gratis — konversi didorong via trial 14 hari pada paket yang dipilih sendiri pendaftar (fitur penuh, kuota mengikuti paketnya; paket Maju dikecualikan karena kuotanya dinegosiasikan — v4.53). Paket **Tumbuh** (250 santri, Rp 349.000) adalah paket paling populer — sweet spot antara harga terjangkau dan kapasitas nyata untuk mayoritas pesantren. Setelah trial berakhir: grace period 7 hari → suspended.*
...
── Manajemen (group) ──
  ...
  Modul / ModulPengaturanPage Squares2x2 (5, slug /admin/pengaturan-modul) [admin_pesantren] (v4.57)
...
> **v4.57 — kelima cluster di atas kini bersyarat `Modul::X->aktif()`.** Cluster Akademik, Tahfidz, Presensi, Kesantrian, dan Keuangan beserta seluruh isinya (termasuk halaman tanpa entri navigasi) hilang dari sidebar, dari tab cluster, dari bottom-nav HP, dan menolak akses URL dengan 403 saat pesantren mematikan modulnya. Halaman **Rapor** top-level juga punya tuasnya sendiri. **Cluster Santri tidak punya tuas** — Santri/Kelas/Kamar/Prestasi adalah inti sistem. Mekanismenya satu: `canAccess()`/`canViewAny()` tiap komponen, yang di Filament sekaligus menjadi saklar navigasi (`Cluster::canAccessClusteredComponents()`) dan penjaga URL **serta hidrasi Livewire** (`CanAuthorizeAccess::hydrateCanAuthorizeAccess()`). Karena itu tidak ada middleware modul di panel admin; kelengkapannya dijaga tes yang mengenumerasi `Cluster::getClusteredComponents()`.
>
> ⚠️ **Koreksi drift v4.26 → v4.57.** Catatan v4.26 di bawah menyatakan bottom nav HP berisi **enam** tab termasuk Presensi. Kodenya tidak pernah begitu: `resources/views/filament/admin/bottom-nav.blade.php` hanya memuat **lima** — Dashboard · Santri · Akademik · Tahfidz · Kesantrian. Tab Presensi tidak pernah ditulis, dan tab Keuangan memang tidak pernah direncanakan. Dokumen ini dikoreksi mengikuti kode; menambahkan tab Presensi adalah perubahan UX tersendiri yang belum diputuskan.
...
**Bottom nav wali (v4.9, bersyarat sejak v4.57):** Beranda · SPP · Pengumuman · Uang Saku · Rapor. Tidak ada tab "Santri" terpisah — "Beranda" merangkap fungsi navigasi ke detail santri (`wali.santri.show`). **SPP** dan **Uang Saku** hilang saat modul Keuangan dimatikan, **Rapor** saat modul Rapor dimatikan; Beranda & Pengumuman tanpa syarat sehingga bar ini tidak pernah kosong. Daftar tabnya dirakit sekali sebagai array lalu di-loop (pola `filament/admin/bottom-nav.blade.php`) — tanpa itu, `flex-1` diam-diam melebarkan tab sisanya setiap kali satu disembunyikan.

**Permukaan wali ⇒ modul *(v4.57)*:**

| Permukaan | Modul | Perilaku saat mati |
|---|---|---|
| `wali.dashboard` · `wali.santri.show` · `wali.pengumuman` | — (inti) | tidak pernah ditutup |
| `wali.santri.tahfidz` | Tahfidz | 404; kartu Capaian Hafalan, Rapor Tahfidz & Riwayat Setoran hilang dari detail santri |
| `wali.santri.kesehatan` · `wali.santri.mutabaah` · `wali.santri.inventaris` | Kesantrian | 404; kartu Amalan & Status Kesehatan, seksi Riwayat Kesehatan/Mutabaah/Inventaris hilang |
| `wali.santri.presensi` · `wali.izin` · `wali.izin.store` · `wali.izin.lampiran` | Presensi | 404; seksi Presensi & Pengajuan Izin dan alert kehadiran di dashboard hilang |
| `wali.rapor` · `wali.laporan.pdf` | Rapor | 404; tab bottom-nav hilang |
| `wali.spp` · `wali.spp.konfirmasi` · `wali.uang-saku` · `wali.uang-saku.show` | Keuangan | 404; dua tab bottom-nav + notifikasi SPP & kartu uang saku di dashboard hilang |
| kartu Ekskul di detail santri | Akademik | koleksinya dikosongkan `SantriDetailPresenter`, `@if`-nya yang lama menutupnya sendiri |
| tab Tahfidz/Karakter/Akademik di `/wali/rapor` | Tahfidz / **Kesantrian** / Akademik | tab hilang; `$activeTab` jatuh ke tab aktif pertama, bukan hardcoded `'tahfidz'` |

> ⚠️ **404, bukan 403** — berbeda dari panel admin yang sengaja 403. Bagi orang tua santri, modul yang pesantrennya tidak pakai memang tidak ada; 403 mengumumkan keberadaan sesuatu yang sedang ditolak dan mengundang pertanyaan tentang fitur yang bukan haknya untuk minta. Di panel admin sebaliknya: yang membuka URL adalah orang yang berwenang menyalakannya kembali.
>
> ⚠️ **Jalur magic link tidak punya `tenant.resolve`.** `SantriDetailPresenter::detail()` membaca modul dari `$santri->pesantren_id`, bukan konteks tenant — alasan yang sama yang membuat query pengumuman di sana memakai `withoutGlobalScope`. Salah di titik ini tidak memunculkan galat apa pun; wali hanya melihat seksi milik pesantren lain.
...
| `modul.diubah` *(v4.57)* | `ModulPengaturanPage::save()` — enam nilai lama & baru sekaligus. Saat seorang ustadz melapor "menu saya hilang", ini satu-satunya jejak yang bisa menjawab siapa mematikan apa dan kapan |
...
**Empty state:** Santri kosong → "tambah santri / import" · Tahfidz → "mulai input setoran" · Mutaba'ah → "gunakan halaman Isi Harian" · Portal Wali santri baru → "data sedang dipersiapkan, cek besok" · **Presensi** *(v4.26)* → tiga guard di halaman Isi Presensi: belum ada santri aktif · **ustadz belum ditetapkan sebagai wali kelas mana pun** ("minta admin menetapkan Anda lewat menu Santri → Kelas") · tanggal terpilih adalah hari libur (peringatan, bukan larangan) · **Modul dimatikan** *(v4.57)* → dua guard Blade baru: halaman **Rapor** panel saat keempat modul isinya mati ("Semua modul rapor sedang dimatikan untuk pesantren ini. Nyalakan lewat menu Manajemen → Modul.") dan **`/wali/rapor`** saat ketiga tabnya mati ("Pesantren belum mengaktifkan satu pun modul rapor."). Yang pertama sengaja **mendahului** cabang "Pilih santri" dan "Centang minimal satu rapor": keduanya menyuruh admin melakukan sesuatu yang tidak akan menolong, dan kalimat yang menyuruh mencentang sesuatu yang tidak ada adalah kalimat yang bohong.
...
| Antrean Demo *(v4.58)* | Excel | Super Admin | Seluruh `demo_requests` (14 kolom), mengikuti filter/pencarian/sortir tabel yang aktif. Kolom SLA memakai derivasi yang sama persis dengan badge di `DemoRequestsTable` |
| Data Pesantren *(v4.58)* | Excel | Super Admin | Satu baris per tenant (26 kolom); admin diambil dari relasi `adminUtama` (admin_pesantren terlama) + kolom Jumlah Admin; santri dihitung live, **bukan** dari `santri_count_cache`. Tenant demo tersembunyi kecuali difilter eksplisit |
...
**Alur kedua — ekspor super admin (v4.58), sama-sama sinkron tapi TANPA rute.** `admin.export.*` tetap empat rute; dua ekspor super admin tidak menambah rute kelima. Tombolnya mengembalikan `BinaryFileResponse` langsung dari closure `Action::make()->action()` di `getHeaderActions()`, dan Livewire (`SupportFileDownloads`) mengubahnya jadi unduhan. Querynya `$this->getTableQueryForExport()`. Alasannya di changelog v4.58: rute adalah request baru yang buta terhadap state filter Livewire, sehingga berkasnya akan selalu berisi seluruh tabel dan diam-diam berbeda dari layar; grup `admin-export` juga bermiddleware `tenant.resolve` yang tidak cocok untuk super admin ber-`pesantren_id = null`. Konsekuensinya: **kelas Export-nya menerima `Builder` dari luar**, bukan scalar `$pesantrenId` seperti empat kelas lama.
...
Validasi dilakukan di dua lapisan: durasi yang tidak memenuhi syarat ditandai `disabled` beserta alasannya di `UpgradePage` dan ditolak `pilihDurasi()` *(v4.54 — pagar ini sebelumnya dipegang filter `->options()` milik `Select` durasi, dan akan hilang tanpa jejak saat dropdown-nya diganti tombol)*, dan `abort_if` di `prosesPembayaran()` sebagai lapisan kedua.

**Downgrade:** satu-satunya pagar yang benar-benar berjalan adalah **kuota** — santri aktif > kuota baru → downgrade diblokir (nonaktifkan santri dulu). *(v4.53: pagar ini sebelumnya hanya tertulis di sini dan tidak ada di kode mana pun. Ia kini ditegakkan `UpgradePage::kuotaKurang()` — tombol bayar mati + callout merah, dan `prosesPembayaran()` menolak lewat Notification karena aksi Livewire tetap bisa dipanggil walau tombolnya mati. Aturan yang sama berlaku pada aksi "Ganti Paket Trial" di `BillingPage`. Sebelum v4.53 arah paket praktis selalu naik — trial selalu Rintisan — sehingga keadaan ini nyaris tidak pernah muncul.)* Penguncian modul saat turun paket (Inventaris, AI, Kesehatan) **tidak ada di kode**: feature lock berbasis paket tidak ditegakkan sama sekali (§5.1), jadi turun paket tidak menutup modul apa pun.
...
- *Service & rules:* `BillingCalculatorService` (formula kuota custom Maju, X=0 di-cover) · `SlugNotReserved` · `ValidTenantSlug` (format/panjang/unik) · `OnboardPesantren` (buat pesantren+admin, trial pada paket pilihan pendaftar dengan kuota dari `BillingCalculatorService`, durasi `BillingSetting::trial_days`, default 14).
...
**Penjaga kelengkapan yang mengenumerasi (v4.57).** `ModulNavigasiTest::test_setiap_komponen_cluster_ikut_dimatikan` tidak menyebut satu pun nama resource: ia memanggil `Cluster::getClusteredComponents()` untuk kelima cluster bertuas, mematikan modulnya, lalu menuntut `canAccess() === false` untuk **setiap** komponen yang dikembalikan. Alasannya bukan kerapian melainkan keterbatasan bahasa: PHP tidak punya mekanisme level-kelas yang anti-override — method yang ditulis di kelas mengalahkan method dari trait **tanpa peringatan apa pun** — sehingga kelengkapan aturan yang tersebar di 27 kelas hanya bisa ditegakkan dari luar hierarki kelasnya. Resource yang ditambahkan ke sebuah cluster tahun depan tercakup sejak hari ia ditulis, tanpa ada yang perlu ingat. Pola ini layak dipakai ulang setiap kali sebuah aturan wajib berlaku untuk *seluruh anggota* sebuah himpunan yang masih akan bertambah. Pasangannya `test_cluster_santri_tidak_pernah_bisa_dimatikan`, yang menjaga arah sebaliknya: `HasAdminOnlyAccess` juga dipakai `KelasResource` & `KamarResource` yang inti, dan yang melindungi keduanya cuma fakta bahwa modul diturunkan dari cluster.
...
```
tests/Feature/                                88 berkas  ← tulang punggung: alur panel Filament,
...
tests/Unit/{Services,Rules}/                   7 berkas  ← kalkulasi & unit murni tanpa DB
                                                            (BillingCalculator, FonnteWhatsApp, SlugRules,
                                                            MutabaahAgregat, 3× Presensi) + ExampleTest
tests/TenantIsolation/                         2 berkas  ← DataIsolationTest + OnboardingIsolationTest,
                                                            wajib lulus sebelum go-live (PostgreSQL)
tests/Concerns/ + tests/Fixtures/              2 berkas  ← helper lintas berkas, bukan tes (v4.51):
                                                            trait MenyemaiWilayah + fixture CSV impor
```
...
| `pesantrens.santri_count_cache` tidak pernah dipelihara *(v4.58)* | Kolom ini hanya pernah **ditulis** di `OnboardPesantren` (diset `0`) dan `SegarkanSandboxCommand`. Komentar di `PesantrenObserver` menyebut ia "di-update setelah santri created/deleted (via SantriObserver opsional)" — observer itu tidak pernah dibuat, jadi nilainya nyaris selalu 0 untuk tenant mana pun. Ia masih dibaca di dua tempat, keduanya tidak berbahaya: modal konfirmasi hapus pesantren & snapshot `old_values` di `ActivityLog` | Saat ada permukaan yang butuh jumlah santri **tanpa** query agregat (mis. daftar tenant yang harus ringan) → pilih satu: hidupkan observernya, atau hapus kolomnya. Jangan biarkan setengah — ekspor Data Pesantren v4.58 sengaja memakai `withCount` dan mengabaikan kolom ini |
| Export admin tidak ikut tuas modul *(v4.57)* | `ExportController` (`admin-export/presensi`, `admin-export/mutabaah`, rute `web.php`) hanya memvalidasi role, jadi tautan yang tersimpan tetap melayani data modul yang sudah dimatikan. Cakupannya sempit — satu-satunya yang bisa mencapainya adalah admin pesantren yang mematikan modulnya sendiri — tapi ia gap yang disengaja, bukan kelalaian | Saat export dipakai lewat penjadwalan/integrasi, atau saat ada peran ketiga yang boleh mengekspor → gate seluruh `ExportController` per modul dalam satu pass, jangan tambal per endpoint |
