...
- **Halaman Rapor (`/wali/rapor`):** filter santri + tahun ajaran, dua tab — "📖 Tahfidz" (nilai per periode & rekomendasi) dan "🌱 Karakter" (penilaian adab 7 item, kepribadian 9 item, catatan ustadz); tombol ekspor PDF siap cetak (`LaporanController::exportPdf`, route `wali.laporan.pdf`, via `barryvdh/laravel-dompdf`). Tab ke-5 di bottom nav wali.
